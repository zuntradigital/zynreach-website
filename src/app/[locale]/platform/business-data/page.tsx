...
const raw = productExperiencePages.find((page) => page.slug === "business-data")!;
const routePath = "/platform/business-data";
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: "productPages.business-data" });
...
export default async function BusinessDataPage({ params }: PageProps) {
  const { locale } = await params;
  setRequestLocale(locale);

  const t = await getTranslations("productPages.business-data");
