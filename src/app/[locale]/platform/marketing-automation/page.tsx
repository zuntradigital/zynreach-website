...
const raw = productExperiencePages.find((page) => page.slug === "marketing-automation")!;
const routePath = "/platform/marketing-automation";
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: "productPages.marketing-automation" });
...
export default async function MarketingAutomationPage({ params }: PageProps) {
  const { locale } = await params;
  setRequestLocale(locale);

  const t = await getTranslations("productPages.marketing-automation");
