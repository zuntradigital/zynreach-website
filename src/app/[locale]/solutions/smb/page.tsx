...
const raw = companySizeSolutionPages.find((page) => page.slug === "smb")!;
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: "companySizeSolutions.smb" });
  const url = "/solutions/smb";
...
export default async function SmbSolutionPage({ params }: PageProps) {
  const { locale } = await params;
  setRequestLocale(locale);

  const t = await getTranslations("companySizeSolutions.smb");
  ...
  const content: CompanySizeSolutionPageContent = {
    slug: raw.slug,
    navLabel: tLinks(hrefToLinkKey["/solutions/smb"] ?? ""),
  ...
  const breadcrumbs = await breadcrumbJsonLd(locale, [
    ...
    { label: content.navLabel, href: "/solutions/smb" },
  ]);
