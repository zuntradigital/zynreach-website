...
const raw = companySizeSolutionPages.find((page) => page.slug === "mid-market")!;
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: "companySizeSolutions.mid-market" });
  const url = "/solutions/mid-market";
...
export default async function MidMarketSolutionPage({ params }: PageProps) {
  const { locale } = await params;
  setRequestLocale(locale);

  const t = await getTranslations("companySizeSolutions.mid-market");
  ...
  const content: CompanySizeSolutionPageContent = {
    slug: raw.slug,
    navLabel: tLinks(hrefToLinkKey["/solutions/mid-market"] ?? ""),
  ...
  const breadcrumbs = await breadcrumbJsonLd(locale, [
    ...
    { label: content.navLabel, href: "/solutions/mid-market" },
  ]);
