...
const raw = companySizeSolutionPages.find((page) => page.slug === "growing-business")!;
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: "companySizeSolutions.growing-business" });
  const url = "/solutions/growing-business";
...
export default async function GrowingBusinessSolutionPage({ params }: PageProps) {
  const { locale } = await params;
  setRequestLocale(locale);

  const t = await getTranslations("companySizeSolutions.growing-business");
  ...
  const content: CompanySizeSolutionPageContent = {
    slug: raw.slug,
    navLabel: tLinks(hrefToLinkKey["/solutions/growing-business"] ?? ""),
  ...
  const breadcrumbs = await breadcrumbJsonLd(locale, [
    ...
    { label: content.navLabel, href: "/solutions/growing-business" },
  ]);
