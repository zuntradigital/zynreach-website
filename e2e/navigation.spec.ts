...
test.describe("Navigation (desktop nav)", () => {
  ...
  test("mega menu opens and links to capability pages", async ({ page }) => {
    ...
    const crmLink = page.getByRole("menuitem", { name: "CRM" });
  ...
  test("Escape closes an open mega menu and returns focus to the trigger", async ({ page }) => {
    ...
    await trigger.click();
    await expect(page.getByRole("menuitem", { name: "CRM" })).toBeVisible();
    await page.keyboard.press("Escape");
    await expect(page.getByRole("menuitem", { name: "CRM" })).toBeHidden();
